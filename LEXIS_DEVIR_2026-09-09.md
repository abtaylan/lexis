...
5. **iOS OTA / Apple-Google ile Giriş / Sınav Hazırlık durumu — 9 Eylül 2026 netleştirildi VE build #10 App Store incelemesine GÖNDERİLDİ (bu oturumun son işi):**
   - **✅ GÖNDERİM YAPILDI (9 Eylül 2026, 22:20 UTC / 01:20 TSİ):** Kullanıcının onayıyla ("Evet, şimdi gönder") App Store Connect'te iOS App Version 1.0.1 oluşturuldu, Build #10 eklendi, App Review Information (giriş bilgileri `mobiltest@test.com`/`Lexis2026Test!`, iletişim, notlar) önceki sürümden aynen taşındığı doğrulandı, "Automatically release this version" seçili olduğu teyit edildi, "Add for Review" → Draft Submission panelinde "Submit for Review" tıklandı. Sonuç ekranı: **"1 Item Submitted — It can take up to 48 hours to be reviewed. You'll get an email when the review is complete."** Sol menüde durum artık **"1.0.1 Waiting for Review"**. Daha önce (bu oturumun başında keşfedildiği üzere) build #10 App Store Connect'e YÜKLENMİŞ ama HİÇ İNCELEMEYE GÖNDERİLMEMİŞTİ ("Ready to Submit" durumundaydı, canlı sürüm hâlâ 1.0/build #8'di) — bu artık düzeltildi, gerçek bir inceleme talebi Apple'a iletildi.
   ...
   - **Ne zaman yansır — somut adımlar (GÜNCEL, gönderim sonrası):** (1) ✅ Build #10 (1.0.1) artık "Waiting for Review" — Apple'ın incelemesi genelde 24-48 saat sürüyor, kullanıcı e-posta ile bilgilendirilecek. (2) İnceleme onaylanınca "Automatically release this version" AÇIK olduğu için Apple otomatik yayına alacak — kullanıcının ayrıca "Release" tıklamasına GEREK YOK. (3) Yayına alınıp cihaz App Store'dan güncelleme aldığında Apple ile Giriş otomatik görünür olur (zaten build içinde). (4) **Yayına alındıktan HEMEN SONRA** (yeni sohbetin ilk işlerinden biri olmalı) `eas update --branch production --platform ios` çalıştırılıp Sınav Hazırlık Alanı + Gramer Rehberi harita UI'ı + "Bugünün Programı" kartı + bu oturumdaki Faz 1 (madde #5) backend değişikliği gibi commit `6cbb70f`'den (build #10) beri biriken TÜM JS değişiklikleri aynı build'e OTA ile gönderilmeli — yeni bir Apple incelemesi GEREKMEZ (runtimeVersion aynı kaldığı sürece). (5) Google ile Giriş ayrı bir geliştirme + yeni native build + yeni Apple incelemesi gerektirir, bu adımların dışında — henüz planlanmadı.
