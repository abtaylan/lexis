...
export const Spinner: React.FC<{ size?: 'sm' | 'md' | 'lg'; className?: string }> = ({
  ...
  return (
    <span
      className={clsx(
        'inline-block border-2 border-sky-200 border-t-sky-500 rounded-full animate-spin',
...
export const EmptyState: React.FC<EmptyStateProps> = ({ icon, title, description, action }) => (
  <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
    {icon && <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400 text-2xl mb-1">{icon}</div>}
...
export const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  return (
    <div className={clsx('flex items-center gap-2', className)}>
      ...
      {showLabel && <span className="text-xs font-medium text-slate-500 w-8 text-right">{pct}%</span>}
    </div>
  );
};
...
export const Select: React.FC<SelectProps> = ({ label, error, wrapperClassName, options, className, ...props }) => (
  <div className={clsx('flex flex-col gap-1.5', wrapperClassName)}>
    ...
    <select
      className={clsx(
        'w-full h-10 rounded-xl border text-sm px-3 bg-white text-slate-800 transition-colors',
        'focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500',
        error ? 'border-red-400' : 'border-slate-200 hover:border-slate-300',
        className
      )}
