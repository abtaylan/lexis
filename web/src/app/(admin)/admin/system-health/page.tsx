...
function StatusPill({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${ok ? 'bg-[#EAF3DE] text-[#3B6D11]' : 'bg-gray-100 text-gray-500'}`}>
...
export default function SystemHealthPage() {
  ...
  if (loading) {
    return <div className="p-8 flex items-center justify-center min-h-[60vh]"><Loader2 className="w-6 h-6 animate-spin text-gray-400" /></div>;
  }
  if (error || !data) return <div className="p-8"><div className="bg-red-50 text-red-600 rounded-2xl px-4 py-3 text-sm">{error}</div></div>;
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Backend, veritabanı, entegrasyonlar ve arka plan işleri</p>
        </div>
        <button onClick={load} className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#534AB7] transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">Backend — çalışıyor</p>
            <p className="text-xs text-gray-500 mt-0.5">Çalışma süresi: {uptimeHours}s {uptimeMinutes}dk · v{data.backend.version}</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${data.database.status === 'ok' ? 'bg-[#E6F1FB] text-[#185FA5]' : 'bg-red-50 text-red-600'}`}><Database className="w-6 h-6" /></div>
          <div>
            <p className="text-sm font-semibold text-gray-900">Veritabanı — {data.database.status === 'ok' ? 'erişilebilir' : 'hata'}</p>
            <p className="text-xs text-gray-500 mt-0.5">Gecikme: {data.database.latency_ms != null ? `${data.database.latency_ms} ms` : '—'}</p>
          </div>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">Entegrasyon durumu</h2>
        ...
        <p className="text-xs text-gray-400 mt-3">&quot;OTP: fixed&quot; ve &quot;Sosyal paylaşım: fixed&quot; test modunda olduğunu gösterir — production&apos;a geçişte &quot;real&quot; yapılması gerekir (bkz. Bölüm 4).</p>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-700">Arka plan işleri (cron script&apos;leri)</h2>
          <p className="text-xs text-gray-400 mt-0.5">Henüz VPS cron&apos;una bağlanmadılar — manuel/lokal çalıştırmalar burada görünür</p>
        </div>
        <div className="divide-y divide-gray-50">
          {data.cron_jobs.map((job) => (
            <div key={job.job_name} className="px-6 py-4 flex items-center justify-between gap-4">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-gray-900">{job.job_name}</p>
                <p className="text-xs text-gray-500 mt-0.5">{JOB_LABELS[job.job_name] || ''}</p>
              </div>
                ...
                {job.last_run ? (
                  <>
                    <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${
                      ...
                      job.last_run.status === 'failed' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-700'
                    }`}>
                    ...
                    <p className="text-xs text-gray-400 mt-1">{formatDate(job.last_run.started_at)}</p>
                  </>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-gray-100 text-gray-400 flex items-center justify-center shrink-0"><Smartphone className="w-6 h-6" /></div>
        <div>
          <p className="text-sm font-semibold text-gray-900">Mobil uygulama — geliştirme aşamasında</p>
          <p className="text-xs text-gray-500 mt-0.5">{data.mobile_app.note}</p>
        </div>
      </div>
