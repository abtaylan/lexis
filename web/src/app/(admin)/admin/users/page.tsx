      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="flex items-center gap-1 text-xs font-medium text-gray-600 mb-1"><GraduationCap className="w-3 h-3" />Öğrenilen dil</label>
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onClose} className="flex-1 border border-gray-200 rounded-xl py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Vazgeç</button>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${detail.role === 'admin' ? 'bg-[#EEEDFE] text-[#534AB7]' : detail.role === 'admin_readonly' ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-500'}`}>{ROLE_LABELS[detail.role] || detail.role}</span>
        ...
        ) : <p className="p-6 text-sm text-gray-400">Kullanıcı bilgileri yüklenemedi.</p>}
      </div>
    </div>
  );
}
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Tüm kullanıcıları görüntüle ve yönet</p>
        </div>
        {!isReadonly && (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-700">Kullanıcı Listesi</h2>
              ...
              {filtered.map((u) => (
                <tr key={u.id} className="border-b border-gray-50 last:border-0 hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${u.role === 'admin' ? 'bg-[#EEEDFE] text-[#534AB7]' : u.role === 'admin_readonly' ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-500'}`}>
                  ...
                  <td className="px-4 py-3 text-xs text-gray-400">{new Date(u.created_at).toLocaleDateString(DATE_LOCALE, { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      {!isReadonly && (
                        <>
                          <button onClick={() => handleRoleToggle(u)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-[#534AB7] hover:bg-[#EEEDFE] transition-colors opacity-0 group-hover:opacity-100" title={u.role === 'admin' ? 'Adminliği kaldır' : 'Admin yap'}>
                            {u.role === 'admin' ? <ShieldOff className="w-3.5 h-3.5" /> : <ShieldCheck className="w-3.5 h-3.5" />}
                          </button>
                          <button onClick={() => handleToggleActive(u)} className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100 ${u.is_active ? 'text-gray-400 hover:text-red-500 hover:bg-red-50' : 'text-gray-400 hover:text-[#3B6D11] hover:bg-[#EAF3DE]'}`} title={u.is_active ? 'Deaktif et' : 'Aktifleştir'}>
