    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">Havuza Kelime Ekle</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-100"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Kaynak dil</label>
              <input value={form.source_lang} onChange={(e) => set('source_lang', e.target.value)} placeholder="en"
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Hedef dil</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Anlam</label>
            <input value={form.meaning} onChange={(e) => set('meaning', e.target.value)}
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Örnek cümle (opsiyonel)</label>
            <input value={form.example} onChange={(e) => set('example', e.target.value)}
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Zorluk (opsiyonel)</label>
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onClose} className="flex-1 border border-gray-200 rounded-xl py-2.5 text-sm font-medium text-gray-600">Vazgeç</button>
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kelime Havuzu</h1>
          <p className="text-sm text-gray-500 mt-0.5">Genel oyun havuzundaki kelimeleri yönet (general_word_pool)</p>
        </div>
        {!isReadonly && (
      ...
      {isReadonly && (
        <div className="flex items-center gap-2 bg-amber-50 text-amber-700 rounded-2xl px-4 py-3 text-sm">
          <Eye className="w-4 h-4 shrink-0" />Salt görüntüleme modundasınız — ekleme/pasif etme işlemleri kapalı.
        </div>
      )}

      {missingCoverage.length > 0 && (
        <div className="flex items-start gap-2 bg-amber-50 text-amber-700 rounded-2xl px-4 py-3 text-sm">
      ...
      <form onSubmit={handleSearch} className="flex flex-wrap gap-3 items-end">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Kaynak dil</label>
          <input value={sourceLang} onChange={(e) => setSourceLang(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm w-24" />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Hedef dil</label>
          <input value={targetLang} onChange={(e) => setTargetLang(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm w-24" />
        </div>
        <div className="flex-1 min-w-[160px]">
          <label className="block text-xs font-medium text-gray-600 mb-1">Kelime ara</label>
          <input value={search} onChange={(e) => setSearch(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm w-full" />
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-700 flex items-center gap-2"><BookOpen className="w-4 h-4" />{sourceLang} → {targetLang}</h2>
            <span className="text-xs font-medium bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{total} kayıt</span>
          </div>
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-gray-400">Bu dil çifti için havuzda kelime yok.</td></tr>
              ) : items.map((it) => (
                <tr key={it.id} className="border-b border-gray-50 last:border-0 hover:bg-slate-50 group">
                  <td className="px-4 py-3 font-semibold text-gray-900">{it.word}</td>
                  <td className="px-4 py-3 text-gray-600 text-xs">{it.meaning}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs truncate max-w-[220px]">{it.example || '—'}</td>
                  <td className="px-4 py-3 text-xs text-gray-500">{it.difficulty_level || '—'}</td>
                  <td className="px-4 py-3 text-right">
                    {!isReadonly && (
                      <button onClick={() => handleDeactivate(it.id)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-colors" title="Pasif et">
